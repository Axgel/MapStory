...
export default function CollaboratorCard(props) {
  ...
  if((auth.user && store.selectedMapOwner) && (auth.user.email === store.selectedMapOwner.email && collaborator.email !== store.selectedMapOwner.email)){
    removeCollabIcon = <img className="w-[25px] h-[25px]" src={closeIcon} alt="" onClick={handleRemoveCollaborator}></img>;
  }
  ...
  return (
    ...
    <div className="w-[280px] h-[40px] rounded-lg shadow-lg bg-white outline-none border-none px-4 my-1 text-lg flex justify-between items-center">
      <div className="flex flex-col items-start">
        ...
        <p className="text-lightgrey text-xs">{ collaborator.email }</p>
      </div>
      <div>
